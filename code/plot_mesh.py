"""Geracao de figuras da malha icosaedral e sua malha dual para o artigo."""
...
def fig_refinement_sequence(levels=(0, 1, 2, 3, 4, 5), outfile="../figures/fig_malha_refinamento.pdf"):
    # espessura de aresta decrescente nos niveis mais finos -- acima de
    # n~4 a malha fica densa demais para uma espessura fixa continuar
    # legivel (o desenho "satura" e parece uma esfera solida).
    ...
    for k, n in enumerate(levels):
        ...
        ax.set_title(f"nível $n={n}$", fontsize=12, y=0.95)
    fig.tight_layout()
    fig.savefig(outfile, bbox_inches="tight")
    print("salvo", outfile)


def fig_primal_vs_dual(level=2, outfile="../figures/fig_malha_primal_dual.pdf"):
    fig = plt.figure(figsize=(12, 4.3))

    ax1 = fig.add_subplot(1, 3, 1, projection="3d")
    plot_primal_mesh(level, ax=ax1)
    ax1.set_title("Malha icosaedral (primal)", fontsize=11, y=0.95)

    ax2 = fig.add_subplot(1, 3, 2, projection="3d")
    plot_dual_mesh(level, ax=ax2)
    ax2.set_title("Malha dual (pentágonos/hexágonos)", fontsize=11, y=0.95)

    ax3 = fig.add_subplot(1, 3, 3, projection="3d")
    plot_primal_and_dual_overlay(level, ax=ax3)
    ax3.set_title("Sobreposição primal/dual", fontsize=11, y=0.95)
...
def fig_dual_cell_detail(level=2, outfile="../figures/fig_celula_dual_detalhe.pdf"):
    """Zoom em uma celula dual hexagonal e uma pentagonal, com rotulos
    condizentes com a notacao da tese (u0, u_j, d_j, h_j)."""
    mesh = build_icosphere(level)
    dm = build_dual_mesh(mesh)

    # celula pentagonal = um dos 12 vertices originais do icosaedro (indice < 12)
    pent_i = 0
    # celula hexagonal = qualquer vertice de indice >= 12
    ...
    for ax, i, label in ((axes[0], pent_i, "pentagonal (m=5)"),
        ...
        for k in range(m):
            ...
            xm, ym = xj / 2, yj / 2  # bissetor (ponto medio) aproximado
        ...
        ax.set_aspect("equal")
        ax.set_title(f"Célula {label}", fontsize=11)
        ax.axis("off")
